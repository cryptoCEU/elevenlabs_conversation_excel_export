      ...
      <main className="max-w-6xl mx-auto px-6 py-8" style={{ display:"flex", flexDirection:"column", gap:20 }}>
        ...
        <div className="card p-6 fade-up">
          <div className="flex items-center gap-2 mb-4">
            ...
            <span style={{ fontWeight:600, fontSize:14 }}>Selecciona agente y rango de fechas</span>
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr auto", gap:12, alignItems:"flex-end" }}>
              ...
              {loadingAgents ? (
                ...
                >
                  <span style={{ overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                    {selectedAgent ? selectedAgent.name : agents.length ? "Seleccionar agente..." : "Sin agentes disponibles"}
                  </span>
                ...
                <div style={{ position:"absolute", top:"calc(100% + 4px)", left:0, right:0, background:"#111118", border:"1px solid var(--border)", borderRadius:8, zIndex:50, overflow:"hidden", boxShadow:"0 8px 32px rgba(0,0,0,0.4)" }}>
                  {agents.map((agent) => (
                    <button key={agent.agent_id}
                      ...
                      style={{ width:"100%", padding:"10px 14px", textAlign:"left", background: selectedAgent?.agent_id === agent.agent_id ? "rgba(108,99,255,0.12)" : "transparent", color: selectedAgent?.agent_id === agent.agent_id ? "var(--accent)" : "var(--text)", border:"none", cursor:"pointer", fontSize:13, fontFamily:"'DM Sans', sans-serif", borderBottom:"1px solid rgba(30,30,46,0.6)", display:"flex", alignItems:"center", gap:8, transition:"background 0.15s" }}
                    >
            ...
            <div>
              ...
              <div style={{ position:"relative" }}>
                ...
                <input className="input" type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} style={{ paddingLeft:34 }} />
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ position:"relative" }}>
                <Calendar size={13} style={{ position:"absolute", left:12, top:"50%", transform:"translateY(-50%)", color:"var(--muted)", pointerEvents:"none" }} />
                <input className="input" type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} style={{ paddingLeft:34 }} />
              </div>
            </div>
          ...
          <div className="fade-up" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:12 }}>
            {[
              ...
              <div key={label} className="card" style={{ padding:"14px 16px" }}>
                <div style={{ display:"flex", alignItems:"center", gap:6, color:"var(--muted)", fontSize:11, marginBottom:6 }}>{icon} {label.toUpperCase()}</div>
        ...
        {!fetched && !loadingAgents && !loadingConvs && agents.length === 0 && !error && (
          <div className="card fade-up" style={{ padding:48, textAlign:"center", color:"var(--muted)" }}>
            <Bot size={32} style={{ margin:"0 auto 12px", opacity:0.3 }} />
            <div style={{ fontSize:14 }}>Cargando agentes...</div>
          </div>
        )}
